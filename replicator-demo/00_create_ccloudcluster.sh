...
# Cluster1
confluent kafka cluster create $XX_CCLOUD_CLUSTERNAME1 --cloud 'gcp' --region 'europe-west1' --type basic --environment $XX_CCLOUD_ENV -o yaml > clusterid1
...
# create API Keys
confluent api-key create --resource $CCLOUD_CLUSTERID1 --description "API Key for cluster user" -o yaml > apikey1
...
# Cluster2
confluent kafka cluster create $XX_CCLOUD_CLUSTERNAME2 --cloud 'aws' --region 'eu-central-1' --type basic --environment $XX_CCLOUD_ENV -o yaml > clusterid2
# set cluster id as parameter
export CCLOUD_CLUSTERID2=$(awk '/id:/{print $NF}' clusterid2)
echo $CCLOUD_CLUSTERID2
confluent kafka cluster use $CCLOUD_CLUSTERID2
confluent kafka cluster describe $CCLOUD_CLUSTERID2 -o yaml > clusterid2
...
# create API Keys
confluent api-key create --resource $CCLOUD_CLUSTERID2 --description "API Key for cluster user" -o yaml > apikey2
...
confluent schema-registry cluster describe -o yaml > srinfos
...
confluent api-key create --resource $CCLOUD_SRID --description "API Key for Replicator show case" -o yaml > srkey
...
kafka-topics --create --bootstrap-server $CCLOUD_CLUSTERID1_BOOTSTRAP --topic cmorders_avro --replication-factor 3 --partitions 6 --command-config ./ccloud_user1.properties 
echo "Topic created"
