...
#curl http://localhost:8083/connectors/replicate-topic/status | jq
...
kafka-avro-console-consumer --bootstrap-server $(awk '/endpoint: SASL_SSL:\/\//{print $NF}' clusterid1 | sed 's/SASL_SSL:\/\///g') --topic cmorders_avro \
